...
// forge-delivery identity and the delivered watermark table.
func TestMigration0018Content(t *testing.T) {
	raw, err := migrations.FS.ReadFile("0018_outbox_forge_versions.sql")
	if err != nil {
	...
	if len(stmts) != 5 {
		t.Fatalf("0018 has %d statements, want 5", len(stmts))
	}
}
