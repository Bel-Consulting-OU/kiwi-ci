...
const schemaSource = `{
  ...
  "$defs": {
    ...
    "shell": {
      "type": "string",
      "enum": ["bash", "sh", "zsh", "pwsh", "powershell", "fish", "dash", "ksh", "python"]
    },
    "trigger": {
      "type": "object",
      "properties": {
        "branches": {"$ref": "#/$defs/stringArray"},
        "branches_ignore": {"$ref": "#/$defs/stringArray"},
        "tags": {"$ref": "#/$defs/stringArray"},
        "tags_ignore": {"$ref": "#/$defs/stringArray"},
        "paths": {"$ref": "#/$defs/stringArray"},
        "paths_ignore": {"$ref": "#/$defs/stringArray"},
    ...
    "retry": {
      ...
      "properties": {
        ...
        "on": {
          "type": "array",
          "items": {"enum": ["failure", "infra", "timeout", "cancelled", "command", "any"]}
        }
      },
      "additionalProperties": false
    },
    ...
    "sandbox": {
      ...
      "properties": {
        ...
        "network": {"enum": ["default", "none", "services-only", "internet"]}
      },
      "additionalProperties": false
    },
    "placement": {
    ...
    "resources": {
      ...
      "properties": {
        ...
        "memory": {"$ref": "#/$defs/byteSize"},
        "disk": {"$ref": "#/$defs/byteSize"},
        "pids": {"type": "integer", "minimum": 0}
      },
      "additionalProperties": false
    },
    "tests": {
    ...
    "deployment": {
      "type": "object",
      "properties": {
        "canary": {"type": "array", "items": {"$ref": "#/$defs/step"}},
        "verify": {"type": "array", "items": {"$ref": "#/$defs/step"}},
        "rollback": {"type": "array", "items": {"$ref": "#/$defs/step"}}
      },
      "additionalProperties": false
    },
    "service": {
    ...
    "cache": {
      ...
      "properties": {
        ...
        "paths": {"$ref": "#/$defs/stringArray"},
        "key": {"type": "string"},
        "hash_files": {"$ref": "#/$defs/stringArray"},
        "restore_keys": {"$ref": "#/$defs/stringArray"}
      },
      "additionalProperties": false
    },
    "artifact": {
    ...
    "job": {
      ...
      "properties": {
        ...
        "if": {"type": "string"},
        "runner": {"$ref": "#/$defs/stringArray"},
        "runtime": {"enum": ["native", "container", "tart"]},
        "image": {"type": "string"},
        "network": {"enum": ["bridge", "host", "none"]},
        "vm": {"type": "string"},
        "shell": {"$ref": "#/$defs/shell"},
        "timeout": {"$ref": "#/$defs/duration"},
        "queue_timeout": {"$ref": "#/$defs/duration"},
        "retry": {"$ref": "#/$defs/retry"},
        "env": {"$ref": "#/$defs/stringMap"},
        ...
        "cache": {"type": "array", "items": {"$ref": "#/$defs/cache"}},
        "artifacts": {"type": "array", "items": {"$ref": "#/$defs/artifact"}},
        "downloads": {"type": "array", "items": {"$ref": "#/$defs/download"}},
        "test_reports": {"$ref": "#/$defs/stringArray"},
        "environment": {"$ref": "#/$defs/environment"},
        "infra_retries": {"type": "integer", "minimum": 0},
        "permissions": {"$ref": "#/$defs/permissions"},
