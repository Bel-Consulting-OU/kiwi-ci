...
// maxJobContainerNameLen bounds the physical job container name (docker's
// hard limit is 255; 200 keeps the same headroom as the service container
// names). Over-long identities are bounded by boundedNormalizedName, which
// hashes the FULL identity, so two runs/jobs sharing a long prefix still get
// distinct containers.
const maxJobContainerNameLen = 200

// lastPhysicalNano makes generated physical names strictly monotonic within
// this process: when the clock has not advanced since the previous name (the
// clock resolution on some hosts is coarser than a nanosecond), the next name
// uses last+1. Combined with the run/job identity in jobContainerName and the
// timestamp in tartCloneName, two resources generated by one process can
// never share a physical name.
var lastPhysicalNano atomic.Int64

// nextPhysicalNano returns a per-process unique, strictly increasing
// nanosecond timestamp suitable for embedding in a physical name.
func nextPhysicalNano() int64 {
	for {
		last := lastPhysicalNano.Load()
		now := time.Now().UnixNano()
		if now <= last {
			now = last + 1
		}
		if lastPhysicalNano.CompareAndSwap(last, now) {
			return now
		}
	}
}

// jobContainerName derives the physical docker name of the job container:
// kiwi-job-<run>-<job>-<unique-nano>. The run/job identity makes concurrent
// jobs on one daemon distinct even if their timestamps collide, the
// monotonic timestamp keeps retries of the same run/job from colliding with
// a previous attempt's name, and boundedNormalizedName hashes the full
// identity instead of truncating it, so two identities sharing a long
// normalized prefix stay distinct.
func jobContainerName(runID, jobID string) string {
	return boundedNormalizedName(fmt.Sprintf("kiwi-job-%s-%s-%d", runID, jobID, nextPhysicalNano()), maxJobContainerNameLen)
}

// dockerCleanupTimeout bounds every post-failure docker cleanup command.
// A wedged daemon must not strand a runner goroutine: cleanup is best-effort
// but always terminates, and a timeout is surfaced as a typed
// ErrDockerCleanupTimeout instead of a silent discard.
var dockerCleanupTimeout = 15 * time.Second

// dockerCleanupWaitDelay additionally bounds the pipe drain after a cleanup
// command is killed, so an orphaned descendant holding the output pipe can
// never extend the cleanup beyond timeout + this grace.
const dockerCleanupWaitDelay = 2 * time.Second

// ErrDockerCleanupTimeout marks a cleanup command that did not finish within
// dockerCleanupTimeout. Callers can errors.Is it to distinguish a genuinely
// stuck daemon from an ordinary command failure.
var ErrDockerCleanupTimeout = errors.New("docker cleanup timed out")

// dockerCleanupCommand runs one best-effort docker cleanup command
// (`rm -f`, `network rm`, ...) under a context that survives parent
// cancellation: cleanup must still run when the job context is already
// canceled, so the parent is detached with context.WithoutCancel and bounded
// by dockerCleanupTimeout instead. On failure the trimmed (and length-bounded)
// command output is included in the returned error; on timeout the error
// wraps both ErrDockerCleanupTimeout and context.DeadlineExceeded.
func dockerCleanupCommand(parent context.Context, docker string, args ...string) error {
	if parent == nil {
		parent = context.Background()
	}
	ctx, cancel := context.WithTimeout(context.WithoutCancel(parent), dockerCleanupTimeout)
	defer cancel()
	...
	if errors.Is(ctx.Err(), context.DeadlineExceeded) {
		return fmt.Errorf("%w after %s: %w: %s %s%s", ErrDockerCleanupTimeout, dockerCleanupTimeout, context.DeadlineExceeded, docker, strings.Join(args, " "), detail)
	}
	return fmt.Errorf("docker cleanup %s %s: %w%s", docker, strings.Join(args, " "), err, detail)
}

// boundedCleanupOutput renders cleanup command output for one diagnostic
// line, truncated so a hostile or verbose daemon cannot flood the log.
func boundedCleanupOutput(out []byte) string {
	const limit = 512
...
func (b *ContainerBackend) StartJob(ctx context.Context, workspace string, emit func(string)) error {
	...
	b.docker = docker
	b.container = jobContainerName(b.RunID, b.JobID)
	network := b.Network
	if network == "" {
		network = "bridge"
	}
