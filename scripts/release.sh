...
#
# Usage: scripts/release.sh [TAG] [--update-formula]
#
# Inputs (env):
...
#                                 to a PEM file. When unset, provenance
#                                 signing is skipped with a warning.
...
if [ -n "${KIWI_RELEASE_SIGNING_KEY:-}" ]; then
	...
	KEY_FILE=""
	echo "release: KIWI_RELEASE_SIGNING_KEY not set; skipping provenance" >&2
fi
